...
| 4 (2026-06-04) | feature: relatorios | `utils/relatorios.js` + `utils/chartTheme.js` + `composables/useRelatorios.js` + `componentes/relatorios/*` (4) + `paginas/suporte/Relatorios.vue` + `rotas/index.js` + `MenuLateralApp.vue` + `paginas/suporte/PainelSuporte.vue` + `package.json` + `utils/__tests__/relatorios.test.js` + `docs/superpowers/specs/2026-06-04-relatorios-painel-design.md` | FEATURE nova: aba Relatorios (perfil suporte) + faixa de resumo no Painel. Brainstorming completo (spec versionada). 5 metricas: volume no tempo (granularidade adaptativa dia/mes), por setor/local, tempo medio de resolucao, taxa de resolucao/backlog, top solicitantes. Periodo por presets (mes/trimestre/ano/tudo). Dados via listener UNICO do suporte (`useRelatorios` le `useNotificacaoSuporte.todos`) -> zero leitura/indice Firestore novo, realtime. Graficos com Chart.js + vue-chartjs tematizados Twilight (`chartTheme.js`). Agregacoes em funcoes puras testadas (Vitest). Painel ganhou faixa com tempo medio + sparkline + link. PRE-REQ: `npm install` (chart.js + vue-chartjs) antes de `npm run dev`/`npm test`. Sem build nesta sessao |
